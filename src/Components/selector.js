...
function Selector({
  ...
  function determineComponent() {
    if ((selectionType === "Killer" || selectionType === "Item") && currentlySelectedKillerOrItem.addOns && Object.keys(currentlySelectedKillerOrItem.addOns).length > 0) {
      ...
      return <div className="result-headline">{message}</div>;
    } else {
      return <div className="result-empty">Not generated yet</div>;
    }
  }

  return (
    <div className="generator-card">
      <button className="generator-card__action" onClick={handleClickEvent}>
        <svg
          className="generator-card__icon"
          width="18"
          height="18"
          viewBox="0 0 24 24"
          aria-hidden="true"
        ...
        <span>{`Get ${selectionType}`}</span>
      </button>
      <div className="generator-card__result selector-container">
        {determineComponent()}
      </div>
    </div>
  );
